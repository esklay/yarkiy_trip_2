 ...
 *           Ленивую загрузку Google-анкеты
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ==========================================================================
     14. ЛЕНИВАЯ ЗАГРУЗКА GOOGLE-АНКЕТЫ
     
     iframe анкеты не имеет атрибута src (только data-src). Источник
     подставляется, когда блок .google-form-wrap подходит к viewport
     (с запасом 200px — форма успевает загрузиться до того, как пользователь
     её увидит). До этого момента браузер не делает запросов к Google
     и не получает её куки — экономия трафика и приватность.
     
     Если IntersectionObserver не поддерживается — подставляем src сразу,
     чтобы форма точно загрузилась (graceful degradation).
     ========================================================================== */
  const googleFormWrap = document.querySelector('.google-form-wrap');
  const googleFormFrame = document.getElementById('googleFormFrame');
  
  if (googleFormWrap && googleFormFrame && googleFormFrame.dataset.src) {
    const loadFrame = () => {
      if (!googleFormFrame.getAttribute('src')) {
        googleFormFrame.setAttribute('src', googleFormFrame.dataset.src);
      }
    };
    ...
    if ('IntersectionObserver' in window) {
      const formObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            loadFrame();
            formObserver.disconnect(); // Грузим только один раз
          }
        });
      }, { rootMargin: '200px 0px' }); // Начинаем загрузку за 200px до появления блока
      
      formObserver.observe(googleFormWrap);
    } else {
